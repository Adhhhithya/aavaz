...
| Referral | Node (S9, extended S10, S16) | Node | INTEGRATED | None (net-new capability, no FastAPI equivalent) | `POST /v1/console/cases/:caseId/referrals`, `GET`/`PATCH /v1/console/referrals/:id`, `GET`/`POST /v1/ack/:token` (public, S16) | `referrals` (S9) + `referrals.in_service_at` (S10) + `referrals.ack_token_hash` (S16, additive) | Unit + integration + concurrency + security | No PDF rendering, no delivery adapter (S16's ack token must be relayed to the receiver manually — no SMS/email integration exists), no automated SLA-breach detection; `legal_aid`/`welfare` packets cannot auto-populate CNR/FIR (not modeled in Node yet); no rate limiting on the public ack endpoint (mitigated by 256-bit token entropy) — see `docs/S16_REFERRAL_ACK_MIGRATION.md` |
...
- **No RLS enforcement** — every table has `ENABLE ROW LEVEL SECURITY`
  ...
  entirely. All authorization is application-layer (S4-S16's consistent
  pattern). Never claimed otherwise anywhere in this repository's docs.
  The ONE exception to "authorization" as a concept at all is `/v1/ack/*`
  (S16), which is deliberately public by design — see
  `docs/S16_REFERRAL_ACK_MIGRATION.md`.
