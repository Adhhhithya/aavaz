        ...
        for event in self._segmenter.feed(samples):
            if event.speech_started:
                ...
                if self._settings.allow_barge_in and self._turn_active():
                    # INFO, not DEBUG: this is the exact event a "TTS cut off
                    # without me interrupting" report is asking about — worth
                    # always being visible, not just under verbose logging.
                    log.info(
                        "session=%s barge-in: new speech detected while a turn "
                        "was still active/speaking, cancelling it",
                        self._session.id,
                    )
                    await self._cancel_turn()
