...
async def lifespan(app: FastAPI):
    ...
    # Both onnxruntime (TTS) and CTranslate2 (ASR) pay a one-time JIT/session
    # warmup cost on their first real inference call — otherwise the very
    # first user of a freshly started server eats ~1-1.5s of extra latency
    # that every subsequent turn doesn't have. Pay that cost now, at startup,
    # on a throwaway input, instead of on someone's real first turn.
    warmup_started = time.perf_counter()
    await app.state.tts.synthesize("Warming up.")
    await app.state.asr.transcribe(np.zeros(16000, dtype=np.float32))
    log.info("model warmup done in %.1fs", time.perf_counter() - warmup_started)
